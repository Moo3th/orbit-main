import { HealthcarePage } from '@/components/business/HealthcarePage';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = getCmsPageById(snapshot, 'healthcare');
  if (page && page.visible === false) return {};

  const seo = extractPageSeo(page, page?.path || '/healthcare');
...
export default async function HealthcarePageRoute() {
  const snapshot = await getSiteCmsSnapshot();
  const cmsPage: CmsPage | null = getCmsPageById(snapshot, 'healthcare') || null;
  ...
  return (
    <>
      <Navbar />
      <HealthcarePage cmsPage={cmsPage} />
      <Footer />
    </>
  );
}
