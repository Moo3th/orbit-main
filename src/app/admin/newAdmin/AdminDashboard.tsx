...
export const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={isAr ? "rtl" : "ltr"} style={{ fontFamily: "'IBM Plex Sans Arabic', sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside className={`fixed ${sidebarEdgeClass} top-0 h-full w-72 max-w-[85vw] ${sidebarOpen ? "lg:w-64" : "lg:w-20"} ${mobileSidebarTranslateClass} bg-[#020817] text-white transition-all duration-500 flex flex-col z-50 shadow-[10px_0_40px_-10px_rgba(0,0,0,0.5)] border-r border-white/5`}>
        <div className="p-6 flex items-center justify-between border-b border-white/5 bg-white/[0.02] backdrop-blur-xl">
          {showSidebarLabels && (
            <motion.div 
            ...
            >
              <div className="bg-gradient-to-br from-primary to-primary/60 p-1.5 rounded-xl shadow-lg shadow-primary/20">
...
const BlogView = ({ isAr, onEditPage }: { isAr: boolean; onEditPage: (id: string) => void }) => {
  ...
  return (
    <div className="space-y-6">
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg text-[#104E8B] font-bold flex items-center gap-2">
            <Newspaper className="w-5 h-5 text-primary" />
            {isAr ? "إدارة المدونة والمحتوى" : "Blog & Content Management"}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-2xl flex items-start gap-3">
             <div className="bg-blue-100 p-2 rounded-xl text-blue-600"><Sparkles className="w-5 h-5" /></div>
             <p className="text-sm text-blue-800 font-medium leading-relaxed">
              {isAr
                ? "اكتب المقالات هنا باستخدام المحرر الاحترافي. تُحفظ المقالات في قاعدة البيانات وتظهر مباشرة في صفحة المدونة العامة."
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            >
              ...
              {isAr ? "تعديل أقسام المدونة" : "Edit Blog Sections"}
            </Button>
            <Button variant="outline" className="border-gray-200 text-gray-700 h-12 rounded-xl" asChild>
        ...
        <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6">
              {isPreviewMode ? (
                <div className="max-w-4xl mx-auto bg-white p-8 border rounded-2xl shadow-inner min-h-[60vh] overflow-y-auto">
                   <div className="flex items-center gap-3 text-xs text-gray-400 mb-4 uppercase tracking-widest">
                     <span className="bg-[#104E8B] text-white px-2 py-0.5 rounded">{draft.category}</span>
                     <span>•</span>
