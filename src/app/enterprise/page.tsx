import { EnterprisePage } from '@/components/business/EnterprisePage';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = getCmsPageById(snapshot, 'enterprise');
  if (page && page.visible === false) return {};

  const seo = extractPageSeo(page, page?.path || '/enterprise');
...
export default async function EnterprisePageRoute() {
  const snapshot = await getSiteCmsSnapshot();
  const cmsPage: CmsPage | null = getCmsPageById(snapshot, 'enterprise') || null;
  ...
  return (
    <>
      <Navbar />
      <EnterprisePage cmsPage={cmsPage} />
      <Footer />
    </>
  );
}
