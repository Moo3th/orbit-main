...
export function EnterprisePage({ cmsPage = null }: EnterprisePageProps) {
  ...
  // Hero
  const heroTitle = getCmsField(cmsPage, 'enterprise-hero', 'title', isRTL, isRTL ? 'حلول اتصالات وموارد بشرية تدعم نمو أعمالك' : 'Business & Enterprise Solutions');
  const heroDescription = getCmsField(cmsPage, 'enterprise-hero', 'description', isRTL, isRTL ? 'نساعد الشركات والمؤسسات على تحسين التواصل الداخلي والخارجي، إدارة الموظفين بكفاءة، وتعزيز تجربة العملاء من منصة واحدة.' : 'We help companies and organizations improve internal and external communication, efficiently manage employees, and enhance customer experience from a single platform.');
  const heroCta1Text = getCmsField(cmsPage, 'enterprise-hero', 'cta1_text', isRTL, isRTL ? 'ابدأ الآن' : 'Get Started Now');
  const heroCta1Url = getCmsField(cmsPage, 'enterprise-hero', 'cta1_url', isRTL, '/request-quote?source=enterprise&serviceType=sms-platform');
  const heroCta2Text = getCmsField(cmsPage, 'enterprise-hero', 'cta2_text', isRTL, isRTL ? 'تواصل مع المبيعات' : 'Contact Sales');
  const heroCta2Url = getCmsField(cmsPage, 'enterprise-hero', 'cta2_url', isRTL, '/request-quote?source=enterprise&serviceType=whatsapp-business-api');

  // Challenges
  const challengesTitle = getCmsField(cmsPage, 'enterprise-challenges', 'title', isRTL, isRTL ? 'التحديات' : 'Challenges');
  const challenges = parseList(getCmsField(cmsPage, 'enterprise-challenges', 'items_json', isRTL, ''), DEFAULT_CHALLENGES);

  // Solutions
  const solutionsTitle = getCmsField(cmsPage, 'enterprise-solutions', 'title', isRTL, isRTL ? 'حلولنا للشركات' : 'Our Solutions for Businesses');
  const solutions = parseList(getCmsField(cmsPage, 'enterprise-solutions', 'items_json', isRTL, ''), DEFAULT_SOLUTIONS);

  // Benefits
  const benefitsTitle = getCmsField(cmsPage, 'enterprise-benefits', 'title', isRTL, isRTL ? 'الفوائد' : 'Benefits');
  const benefits = parseList(getCmsField(cmsPage, 'enterprise-benefits', 'items_json', isRTL, ''), DEFAULT_BENEFITS);

  // Final CTA
  const ctaTitle = getCmsField(cmsPage, 'enterprise-cta', 'title', isRTL, isRTL ? 'ابدأ الآن' : 'Get Started Now');
  const ctaCta1Text = getCmsField(cmsPage, 'enterprise-cta', 'cta1_text', isRTL, isRTL ? 'ابدأ الآن' : 'Get Started Now');
  const ctaCta1Url = getCmsField(cmsPage, 'enterprise-cta', 'cta1_url', isRTL, '/request-quote?source=enterprise&serviceType=sms-platform');
  const ctaCta2Text = getCmsField(cmsPage, 'enterprise-cta', 'cta2_text', isRTL, isRTL ? 'تواصل مع المبيعات' : 'Contact Sales');
  const ctaCta2Url = getCmsField(cmsPage, 'enterprise-cta', 'cta2_url', isRTL, '/request-quote?source=enterprise&serviceType=whatsapp-business-api');

  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      {/* Hero Section */}
      {isSectionVisible('enterprise-hero') && (
      ...
      {/* Challenges Section */}
      {isSectionVisible('enterprise-challenges') && (
      ...
      {/* Solutions Section */}
      {isSectionVisible('enterprise-solutions') && (
      ...
      {/* Benefits Section */}
      {isSectionVisible('enterprise-benefits') && (
      <section className="relative py-24 bg-gray-50 dark:bg-gray-800 overflow-hidden">
        <OrbitSectionBackground alignment="both" density="low" />
      ...
      {/* CTA Section */}
      {isSectionVisible('enterprise-cta') && (
